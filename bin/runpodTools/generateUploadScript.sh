#!/usr/bin/env bash
# generateUploadScript.sh
# Generate uploadModels.sh script for uploading models to RunPod

set -euo pipefail

OUTPUT_FILE="${1:-./uploadModels.sh}"

cat > "$OUTPUT_FILE" <<'UPLOAD_SCRIPT'
#!/usr/bin/env bash
...
chmod +x "$OUTPUT_FILE"
echo "Generated upload script: $OUTPUT_FILE"
