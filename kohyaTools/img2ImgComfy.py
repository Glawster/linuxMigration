...
- Uses the appropriate workflow for each image type (fullbody_api.json, halfbody_api.json, portrait_api.json).
...
- Downloads outputs into --runsdir/run_<stamp>/{bucket}/{stem}/...
...
- Saves outputs into --runsdir/run_<stamp>/{bucket}/{stem}/... and mirrors fixed_* files into --comfyout
...
def getAvailableNodes(session, baseUrl):
    """Fetch and return the dict of available node types from the ComfyUI /object_info endpoint."""
    r = session.get(f"{baseUrl}/object_info", timeout=30)
    r.raise_for_status()
    return r.json()

def validateWorkflowNodes(workflow: dict, availableNodes: dict, logger):
    """Raise RuntimeError if the workflow references node class_types not registered on the server."""
    missing = set()
...
class ComfyClient:
    def __init__(self, baseUrl: str, timeoutSeconds: int, *, isRemote: bool):
        """Initialise the client with a base URL, request timeout, and remote/local flag."""
...
def parseArgs(cfg: Dict[str, Any]) -> argparse.Namespace:
    """Parse CLI arguments for the img2img workflow runner, filling defaults from cfg."""
...
def getWorkflowPaths(cfg: Dict[str, Any], workflowsDir: Path) -> Dict[str, Path]:
    """Get all three workflow paths (fullbody, halfbody, portrait)."""
...
def writeOutputs(
    *,
    ...
    """Download and save workflow output images, optionally mirroring them as fixed_* files."""
    ...
    for n, meta in enumerate(images, start=1):
        ...
        if mirrorDir is not None:
            ...
            # Mirror: write a "fixed_" file into mirrorDir for downstream scripts
...
def main() -> int:
    """Load config, parse args and run img2img workflows locally or on a remote ComfyUI instance."""
    cfg = loadConfig()
    args = parseArgs(cfg)
    dryRun = True
    if args.confirm:
    ...
    workflowPaths = getWorkflowPaths(cfg, workflowsDir)
    ...
    # Setup bucket rules for image classification
    rules = [
        BucketRules(
            "fullbody",
    ...
    for img in sorted(imagesToProcess):
        ...
        # Classify image to determine which workflow to use
