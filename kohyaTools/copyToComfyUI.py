...
- scan trainingRoot for images (ignores backup files with .orig or __orig in filename)
- detect faces
...
def iterImages(root: Path, skipDirs: set[str]) -> Iterable[Path]:
    for dirPath, dirNames, fileNames in os.walk(root):
        ...
        for name in fileNames:
            p = Path(dirPath) / name
            # Skip files with .orig or __orig in the filename
            # These are backup/processed files that should not be re-copied
            # Examples: file.orig, file.orig_01, file__orig.jpg, file__orig_001.jpg
            name_lower = name.lower()
            if ".orig" in name_lower or "__orig" in name_lower:
                continue
